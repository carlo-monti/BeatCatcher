...
void Sync::updateSync(){
  ...
  if(_ons->isSnare[_ons->currentOnset]){
    /* per stampa */
    onsetKickPerStampa = 0;
    onsetSnarePerStampa = 1;
    /* per stampa*/
    //Serial.println("SYNC Snare -----------");
    lSync = _lSyncSnare[_clk->barPosition];
  }else{
    /* per stampa */
    onsetKickPerStampa = 1;
    onsetSnarePerStampa = 0;
    /* per stampa*/
  ...
  if(accuracy > _thetaSync){
    ...
    if(thisLayer >= _currentHigherLayer){ // no sync but change parameters
      ...
      _sigmaSync = max((_clk->tau) / 20,_sigmaSync * (_expandWindowRatio + ((0.7 * lSync) - accuracy))); // calculate sigma from current tau
...
void Sync::updateLayer(){
  if(_lastBarPosition != _clk->barPosition){ // whenever bar position changes do:
    ...
    if(_accuracyOfLastSync[_lastBarPosition] == 0){ // if no onset to be synced detected in last bar position decrease current higher layer
  ...
  if(curr > lastMillisForPrint){
    int thisLayer = _clk->layerOf[_clk->barPosition]; // layer of this onset
    long deltaTauSync = 0;
    long error = (curr - _clk->expectedClick); // current onset - expected onset (tn - E[tn])
    float accuracy;
    float gaussian;
    if(error >= ((_clk->tau) / 3)){ // notch for 16th
      gaussian = 0;
